          ...
          <div className="flex flex-wrap justify-center gap-2 md:gap-3">
            {CATEGORIES.map((c) => (
              <button
                ...
                className={`px-4 py-2 text-xs uppercase tracking-widest rounded-full border transition-all duration-500 ${
                  category === c
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {projects.length === 0 ? (
            ...
            <>
              ...
              <div className="md:hidden space-y-6">
                {projects.map((item, i) => (
                  <button
                    ...
                    className={`group block w-[88%] relative overflow-hidden bg-background shadow-xl rounded-sm text-left ${
                      i % 2 === 0 ? "ml-0" : "ml-auto"
                    }`}
                  ...
                  >
                    <div className="aspect-[4/3] overflow-hidden">
                      {item.main_image_url ? (
                        <img src={item.main_image_url} alt={item.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center bg-brand-slate text-brand-gold/40 font-display text-2xl">
                          GELINHOO
                        </div>
                      )}
                    </div>
                    <div className="p-4">
                      ...
                      <h3 className="font-display text-2xl text-brand-slate mt-1 leading-tight">
                        {item.title}
                      </h3>
                      {item.location && (
                        <p className="text-muted-foreground text-xs flex items-center gap-1.5 mt-2 font-light">
              ...
              {/* DESKTOP: GRID */}
              <div className="hidden md:grid grid-cols-2 md:grid-cols-3 gap-4 auto-rows-[280px]">
                {projects.map((item) => (
                  <button
                    key={item.id}
                    onClick={() => setOpenId(item.id)}
                    className={`group relative overflow-hidden cursor-pointer bg-background shadow-lg rounded-lg text-left ${item.grid_class ?? ""}`}
                  >
                    {item.main_image_url ? (
                      <img src={item.main_image_url} alt={item.title} className="object-cover w-full h-full transition-transform duration-1000 ease-out group-hover:scale-105" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center bg-brand-slate text-brand-gold/40 font-display text-2xl">
                        GELINHOO
                      </div>
                    )}
                    <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 flex flex-col justify-end p-5">
            ...
            <>
              ...
              <div className="aspect-video w-full overflow-hidden bg-brand-gray">
                {openProject.project.main_image_url && (
                  <img src={openProject.project.main_image_url} alt={openProject.project.title} className="w-full h-full object-cover" />
                )}
              </div>
              <div className="p-6 md:p-8">
                <DialogHeader>
                  ...
                  <DialogTitle className="font-display text-3xl md:text-4xl text-brand-slate mt-2">
                  ...
                  {openProject.project.location && (
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-brand-slate font-medium">{openProject.project.location}</p>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {openProject.images.map((img) => (
                        <div key={img.id} className="aspect-square overflow-hidden rounded-sm">
                          <img src={img.image_url} alt={img.caption ?? ""} className="w-full h-full object-cover hover:scale-105 transition-transform" />
                        </div>
                      ))}
                    </div>
                  </div>
